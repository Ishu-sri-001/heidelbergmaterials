...
let tiltDirZ = 1;
  ...
  if (!isZoomed) {
    ...
    ref.current.rotation.y = baseRotation.current.y;
   
    ref.current.rotation.z += 0.001 * tiltDir;

    if (ref.current.rotation.z > baseRotation.current.z + 0.08) tiltDir = -1;
    if (ref.current.rotation.z < baseRotation.current.z - 0.08) tiltDir = 1;
  } else {
    ...
    ref.current.rotation.y += 0.001 * tiltDirZ;

    if (ref.current.rotation.y > zoomedBaseRotation.y + 0.1) tiltDirZ = -1;
    if (ref.current.rotation.y < zoomedBaseRotation.y - 0.1) tiltDirZ = 1;
  }
});
