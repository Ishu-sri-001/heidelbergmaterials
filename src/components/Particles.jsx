...
function ParticleField({ count = 5000, radius = 4, color = "green", size = 0.02, opacity = 0.5, rotationX = 25, rotationY = 30 }) {
    const ref = useRef()
...
function ClusterParticleField({
    ...
    const [subPositions] = useState(() => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const subs = random.inSphere(
                new Float32Array(subParticleCount * 3),
    ...
    return (
        <group rotation={[0, 0, Math.PI / 4]}>
            {/* Main Particles */}
            <Points ref={mainRef} positions={mainPositions} stride={3} frustumCulled={false}>
                <PointMaterial
                    transparent
                    color={color}
                    size={mainSize}
            ...
            {/* Sub Particles */}
            <Points ref={subRef} positions={subPositions} stride={3} frustumCulled={false}>
                <PointMaterial
                    // transparent
                    color={subColor}
                    size={subSize}
...
export default function Scene() {
    return (
        <div className='h-screen w-full relative bg-gradient-to-l from-[#05381D] to-[#1A693F]'>
            <Canvas camera={{ position: [0, 0, 0] }}>
                <ParticleField size={0.01} count={5000} color='#6BA32C' opacity={.5} />
                <ParticleField count={3000} radius={4.1} color="#C0EC82" size={0.008} opacity={1} rotationX={30} rotationY={40} />
                <ParticleField count={3000} radius={3.9} color="green" size={0.008} opacity={.8} rotationX={30} rotationY={40} />
                <ParticleField count={1000} radius={3.8} color="black" size={0.012} opacity={1} rotationX={30} rotationY={40} />
                <ClusterParticleField count={200} subColor='white' />
                <EffectComposer>
                    <Vignette
                        offset={0.2}
                        darkness={0.2}
